...
def search_materials_project_by_formula(
    ...
    try:
        ...
        if result is None or (isinstance(result, list) and len(result) == 0):
            return (
                f"Error: No structure found for formula {formula} in Materials Project."
            )
        ...
        if return_all:
            ...
            if save_to_file:
                ...
                safe_name = (
                    formula.replace("-", "").replace(" ", "").replace("*", "_star")
                )
...
def search_materials_project_by_chemsys(
    chemsys: str, api_key: Optional[str] = None, save_to_file: Optional[str] = None
) -> str:
...
def visualize_structure(
    ...
    try:
        ...
        if input_path.is_dir():
            ...
            for struct_file in structure_files:
                try:
                    ...
                    pio.write_image(
                        fig, img_path, width=width, height=height, scale=scale
                    )
        ...
        is_trajectory = any(
            str(input_path).endswith(ext) for ext in trajectory_extensions
        )
        ...
        if output_path:
            ...
            if (
                not out_p.is_absolute()
                and len(out_p.parts) == 1
                and current_research_dir
            ):
    ...
    try:
        ...
        for i, work in enumerate(results, 1):
            ...
            if download and doi and doi != "No DOI":
                ...
                if (
                    "elsevier"
                    in (work.get("primary_location") or {})
                ...
                downloaded_path = download_paper_by_publisher(
                    doi, publisher_hint, output_dir
                )
                if downloaded_path:
                    summary += f"- **Downloaded Full-Text:** {downloaded_path}\\n"
                else:
                    summary += (
                        "- **Download Failed:** Hit paywall or access restriction.\\n"
                    )
...
def supercell_expansion(
    ...
    try:
        ...
        return (
            f"Error executing supercell_expansion: {str(e)}\n{traceback.format_exc()}"
        )


@mcp.tool()
def modify_structure(
    structure_path: str, substitution_dict_json: str, save_to_file: Optional[str] = None
) -> str:
