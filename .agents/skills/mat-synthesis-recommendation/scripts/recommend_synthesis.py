        ...
        try:
            ...
            for recipe in recipes:
                ...
                if synthesis_type and recipe_dict.get("synthesis_type"):
                    if (
                        synthesis_type.lower()
                        not in recipe_dict["synthesis_type"].lower()
                    ):
                        continue
                        ...
                        if isinstance(op, dict) and "conditions" in op:
                            for condition in op["conditions"]:
                                if (
                                    ...
                                    and condition.get("type") == "temperature"
                                ):
...
def main():
    ...
    parser.add_argument(
        "formula", help="Target material formula (e.g., 'LiFePO4', 'Li2CO3')"
    )
    parser.add_argument(
        "--type",
        dest="synthesis_type",
    ...
    parser.add_argument(
        "--min-temp", type=float, help="Minimum synthesis temperature in °C"
    )
    parser.add_argument(
        "--max-temp", type=float, help="Maximum synthesis temperature in °C"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--api-key", help="Materials Project API key (overrides environment variable)"
    )
    ...
    if not api_key:
        ...
        print(
            "Please set MP_API_KEY environment variable or configure ~/.atomistic_skills.yaml"
        )
        sys.exit(1)
